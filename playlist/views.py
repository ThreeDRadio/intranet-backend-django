...
def summary(request):
    ...
    if reportFormat == "top20":
        ...
        songs = (
            PlaylistEntry.objects.filter(playlist__date__range=(startDate, endDate))
            .values("artist", "title", "duration")
            .annotate(plays=Count("duration"))
            .order_by("artist", "title", "-plays")
        )
...
class ShowViewSet(viewsets.ModelViewSet):
    ...
    def topartists(self, request, pk=None):
        ...
        top = (
            PlaylistEntry.objects.filter(playlist__show=show)
            .values("artist")
            .annotate(plays=Count("artist"))
            .order_by("-plays")[:10]
        )
    ...
    def statistics(self, request, pk=None):
        ...
        local = (
            PlaylistEntry.objects.filter(playlist__show=show).filter(local=True).count()
        )
        australian = (
            PlaylistEntry.objects.filter(playlist__show=show)
            .filter(australian=True)
            .count()
        )
        female = (
            PlaylistEntry.objects.filter(playlist__show=show)
        ...
        artists = (
            PlaylistEntry.objects.filter(playlist__show=show).distinct("artist").count()
        )

        data = (
            {"name": "Total tracks", "value": tracks},
            {"name": "Unique artists", "value": artists},
            {"name": "Local", "value": local},
            {"name": "Australian", "value": australian},
            {"name": "Female", "value": female},
        )

        serializer = ShowStatisticsSerializer(
    ...
    def playlists(self, request, pk=None):
        ...
        serializer = PlaylistSerializer(
            show.playlists.all().order_by("-date"),
...
class PlaylistViewSet(viewsets.ModelViewSet):
    ...
    def tracks(self, request, pk=None):
        ...
        serializer = PlaylistEntrySerializer(
            post.tracks.all().order_by("index", "pk"),
            context={"request": request},
            many=True,
        )
        return Response(serializer.data)
...
class PlaylistEntryViewSet(viewsets.ModelViewSet):
    ...
    def today(self, request):
        queryset = (
            ...
            .order_by("artist", "-plays")
        )
        page = self.paginate_queryset(queryset)
        if page is not None:
            serializer = PlayCountSerializer(
