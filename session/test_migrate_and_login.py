...
class MigrateAndLoginTest(APITestCase):
    def setUp(self):
        ...
        OldPassword.objects.create(
            user=self.oldUser, password=md5("pass2".encode("utf-8")).hexdigest()
        )
    ...
    def test_user_with_new_password_authenticates(self):
        ...
        request = factory.post(
            "/api-token-auth", {"username": "newUser", "password": "pass1"}
        )
    ...
    def test_user_with_old_password_authenticates_and_migrates(self):
        ...
        factory = APIRequestFactory()
        request = factory.post(
            "/api-token-auth", {"username": "oldUser", "password": "pass2"}
        )
        view = MigrateAndLogin.as_view()
        response = view(request)
        self.assertEqual(response.status_code, 200)
